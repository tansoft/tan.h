...
SKINPPWTL_API BOOL skinppLoadSkinFromRes(HINSTANCE hInstance,LPCSTR szResourceName,LPCSTR szResourceType,char* szSkinFileName);
...
//szName		: 皮肤资源名称
//HBITMAP		: 返回资源中的位图
SKINPPWTL_API HBITMAP skinppGetBitmapRes(LPCSTR szName);

//通过资源名称取资源的内存指针
//szName		: 资源名称
//nSize			: 资源大小
//pByte			: 返回值，成功返回非NULL，失败返回NULL
SKINPPWTL_API BYTE*  skinppGetSkinResource(LPCSTR szName,int& nSize);
...
SKINPPWTL_API BOOL skinppGetBitmapResRect(LPCSTR szName,int& nTopHeight,int& nBottomHeight,
...
SKINPPWTL_API void skinppSetSkinHwnd(HWND hWnd,LPCSTR szClassName = NULL);
