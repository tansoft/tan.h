...
#include <afxdisp.h>
#include "THDebug.h"
#include "THResizeDialogRect.h"

class THFlash
	: public CWnd
{
protected:
	DECLARE_DYNCREATE(THFlash)
public:
	THFlash()
	{
		m_bHaveSizeArrow=FALSE;
		m_nNcRightClickMsg=0;
		m_nNcDblClickMsg=0;
		m_nTitleHeight=-1;
	}
	CString CallFunction(LPCTSTR request)
	{
		...
		{
			THDebug(_T("[Flash]:Warning:FlashCall Dispatch Exception in Call:%s"),request);
			e->Delete();
		}
		catch(CException *e)
		{
			THDebug(_T("FlashCall Exception in Call:%s"),request);
			ASSERT(FALSE);
			e->Delete();
		}
		return result;
	}
	CString CallFunctionEx(CString fnName)
	{return CallFunction(_T("<invoke name=\"")+fnName+_T("\" returntype=\"xml\"></invoke>"));}
	CString CallFunctionEx1(CString fnName,CString arg1)
	{return CallFunction(_T("<invoke name=\"")+fnName+_T("\" returntype=\"xml\"><arguments><string>")+arg1+_T("</string></arguments></invoke>"));}
	CString CallFunctionEx2(CString fnName,CString arg1,CString arg2)
	{return CallFunction(_T("<invoke name=\"")+fnName+_T("\" returntype=\"xml\"><arguments><string>")+arg1+_T("</string><string>")+arg2+_T("</string></arguments></invoke>"));}
	CString CallFunctionEx3(CString fnName,CString arg1,CString arg2,CString arg3)
	{return CallFunction(_T("<invoke name=\"")+fnName+_T("\" returntype=\"xml\"><arguments><string>")+arg1+_T("</string><string>")+arg2+_T("</string><string>")+arg3+_T("</string></arguments></invoke>"));}
	CString CallFunctionEx4(CString fnName,CString arg1,CString arg2,CString arg3,CString arg4)
	{return CallFunction(_T("<invoke name=\"")+fnName+_T("\" returntype=\"xml\"><arguments><string>")+arg1+_T("</string><string>")+arg2+_T("</string><string>")+arg3+_T("</string><string>")+arg4+_T("</string></arguments></invoke>"));}
	...
	BOOL ParseNcMessage(UINT nFlags, CPoint point);
	afx_msg void OnChar(UINT nChar, UINT nRepCnt, UINT nFlags);
	afx_msg void OnSetFocus(CWnd* pOldWnd);
public:
	BOOL m_bHaveSizeArrow;				///<是否在Flash边缘显示改变大小的箭头
	UINT m_nNcRightClickMsg;			///<是否在右键时，向上级响应消息
	UINT m_nNcDblClickMsg;				///<是否在双击时，向上级响应消息
	THResizeDialogRect m_rect;			///<是否进行特殊控件的单独控制，添加到这里的控件区域不响应大小改变等消息
	int m_nTitleHeight;					///<标题栏高度，设定标题多高响应标题栏消息，-1为总是响应
