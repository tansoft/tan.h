...
{
  unsigned bits = (unsigned)mpz_sizeinbase(x, 2);
		       ...
		       time_t timestamp)
{
  ...
  if (! (pgp_put_header(buffer, PGP_TAG_PUBLIC_KEY,
	 ...
	 && pgp_put_uint32(buffer, (uint32_t)timestamp)/* Time stamp */
...
#define WRITE(buffer, s) (nettle_buffer_write(buffer, (unsigned int)strlen((s)), (s)))
